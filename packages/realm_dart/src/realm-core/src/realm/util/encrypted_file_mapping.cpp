...
// Dynamic encryption page size that adapts to system page size
// Minimum 4096 bytes for compatibility, but scales up for 16KB page systems
inline uint16_t get_encryption_page_size() {
...
// Legacy constant for static assertions and backward compatibility checks
constexpr uint16_t legacy_encryption_page_size = 4096;

constexpr uint8_t metadata_size = sizeof(IVTable);

// Dynamic pages per block calculation
inline uint8_t get_pages_per_block() {
    return static_cast<uint8_t>(get_encryption_page_size() / metadata_size);
}
...
inline size_t page_index(SizeType data_pos) noexcept
{
    SizeType index = data_pos / get_encryption_page_size();
    return assert_cast<size_t>(index);
}

// Number of pages required to store `size` bytes
inline size_t page_count(SizeType size) noexcept
{
    auto page_size = get_encryption_page_size();
    return assert_cast<size_t>((size + page_size - 1) / page_size);
}

// Index of the metadata block which contains `data_pos`
inline size_t block_index(SizeType data_pos) noexcept
{
    return page_index(data_pos) / get_pages_per_block();
}

// Number of metadata blocks required to store `size` bytes
inline size_t block_count(SizeType data_size) noexcept
{
    auto ppb = get_pages_per_block();
    return (page_count(data_size) + ppb - 1) / ppb;
}
...
SizeType data_pos_to_file_pos(SizeType data_pos)
{
    ...
    return data_pos + (block_index(data_pos) + 1) * get_encryption_page_size();
}
...
SizeType file_pos_to_data_pos(SizeType file_pos)
{
    ...
    auto ppb = get_pages_per_block();
    const size_t metadata_page_count = (page_index(file_pos) + ppb) / (ppb + 1);
    return file_pos - metadata_page_count * get_encryption_page_size();
}
...
SizeType iv_table_pos(SizeType data_pos)
{
    ...
    auto ppb = get_pages_per_block();
    const size_t metadata_index = index & (ppb - 1);
    return metadata_block * (ppb + 1) * get_encryption_page_size() + metadata_index * metadata_size;
}
...
{
    REALM_ASSERT(data_pos >= 0);
    return block_index(data_pos) * (get_pages_per_block() + 1) * get_encryption_page_size();
}
...
{
    return File::read_static(fd, pos, static_cast<char*>(dst), get_encryption_page_size());
}

// first block is iv data, second page is data
// Note: c_min_encrypted_file_size validation is now runtime due to dynamic page size
// static_assert(c_min_encrypted_file_size == 2 * legacy_encryption_page_size,
//               "changing the block size breaks encrypted file portability");
...
AESCryptor::AESCryptor(const char* key)
    ...
    , m_rw_buffer(new char[get_encryption_page_size()])
    , m_dst_buffer(new char[get_encryption_page_size()])
{
...
{
    hmac_sha224(Span(reinterpret_cast<const uint8_t*>(m_rw_buffer.get()), get_encryption_page_size()), hmac,
...
bool AESCryptor::refresh_iv(FileDesc fd, size_t page_ndx)
{
    ...
    if (page_ndx >= m_iv_buffer.size() || !m_iv_blocks_read[page_ndx / pages_per_block]) {
        read_iv_block(fd, SizeType(page_ndx) * get_encryption_page_size());
    }
                                                ...
                                                uint32_t& iv_out, Hmac& hmac)
{
    ...
    if (actual < get_encryption_page_size()) {
    ...
    memcpy_if_changed(dst, m_dst_buffer.get(), get_encryption_page_size());
...
void AESCryptor::try_read_block(FileDesc fd, SizeType pos, char* dst) noexcept
{
    ...
    if (bytes_read == 0) {
        ...
        memset(dst, 0x55, get_encryption_page_size());
        return;
    }
    ...
    if (iv.iv1 == 0) {
        ...
        memset(dst, 0xAA, get_encryption_page_size());
        return;
    }
...
void AESCryptor::write(FileDesc fd, SizeType pos, const char* src, WriteMarker* marker) noexcept
{
    ...
    do {
        ...
        hmac_sha224(Span(reinterpret_cast<uint8_t*>(m_rw_buffer.get()), get_encryption_page_size()), iv.hmac1,
                    Span(m_key).sub_span<32>());
    ...
    File::write_static(fd, data_pos_to_file_pos(pos), m_rw_buffer.get(), get_encryption_page_size());
    if (marker)
        marker->unmark();
...
void AESCryptor::crypt(EncryptionMode mode, SizeType pos, char* dst, const char* src, const char* stored_iv) noexcept
{
    ...
    CCCryptorStatus err =
        auto page_size = get_encryption_page_size();
        CCCryptorUpdate(cryptor, src, page_size, dst, page_size, &bytesEncrypted);
    REALM_ASSERT(err == kCCSuccess);
    REALM_ASSERT(bytesEncrypted == get_encryption_page_size());
#elif defined(_WIN32)
    ULONG cbData;
    int i;
    ...
    if (mode == mode_Encrypt) {
        auto page_size = get_encryption_page_size();
        i = BCryptEncrypt(m_aes_key_handle, (PUCHAR)src, page_size, nullptr, (PUCHAR)iv, sizeof(iv),
                          (PUCHAR)dst, page_size, &cbData, 0);
        REALM_ASSERT_RELEASE_EX(i == 0 && "BCryptEncrypt()", i);
        REALM_ASSERT_RELEASE_EX(cbData == get_encryption_page_size() && "BCryptEncrypt()", cbData);
    }
    else if (mode == mode_Decrypt) {
        auto page_size = get_encryption_page_size();
        i = BCryptDecrypt(m_aes_key_handle, (PUCHAR)src, page_size, nullptr, (PUCHAR)iv, sizeof(iv),
                          (PUCHAR)dst, page_size, &cbData, 0);
        REALM_ASSERT_RELEASE_EX(i == 0 && "BCryptDecrypt()", i);
        REALM_ASSERT_RELEASE_EX(cbData == get_encryption_page_size() && "BCryptDecrypt()", cbData);
    }
    else {
        REALM_UNREACHABLE();
    }
...
EncryptedFileMapping::EncryptedFileMapping(EncryptedFile& file, SizeType file_offset, void* addr, size_t size,
    ...
    , m_marker(marker)
#ifdef REALM_DEBUG
    , m_validate_buffer(new char[get_encryption_page_size()])
#endif
{
...
{
    return reinterpret_cast<uintptr_t>(addr) & (get_encryption_page_size() - 1);
}
...
size_t EncryptedFileMapping::get_local_index_of_address(const void* addr, size_t offset) const noexcept
{
    ...
    return (reinterpret_cast<uintptr_t>(addr) - reinterpret_cast<uintptr_t>(m_addr) + offset) / get_encryption_page_size();
}
...
char* EncryptedFileMapping::page_addr(size_t local_ndx) const noexcept
{
    ...
    return static_cast<char*>(m_addr) + (local_ndx * get_encryption_page_size());
}

SizeType EncryptedFileMapping::page_pos(size_t local_ndx) const noexcept
{
    return SizeType(local_ndx + m_first_page) * get_encryption_page_size();
}
...
bool EncryptedFileMapping::copy_up_to_date_page(size_t local_ndx) noexcept
{
    ...
    for (auto& m : m_file.mappings) {
        ...
        memcpy_if_changed(page_addr(local_ndx), m->page_addr(other_mapping_ndx), get_encryption_page_size());
...
void EncryptedFileMapping::write_and_update_all(size_t local_ndx, uint16_t offset, uint16_t size) noexcept
{
    ...
    REALM_ASSERT(offset + size <= get_encryption_page_size());
    ...
    for (auto& m : m_file.mappings) {
        ...
        // page may be out of date
            ...
            memcpy_if_changed(m->page_addr(other_local_ndx), page_addr(local_ndx), get_encryption_page_size());
            set(state, UpToDate);
            clear(state, StaleIV);
        }
    }
...
void EncryptedFileMapping::validate_page(size_t local_ndx) noexcept
{
    ...
    for (auto& m : m_file.mappings) {
        ...
        if (m != this && m->contains_page(ndx_in_file) && is(m->m_page_state[other_local_ndx], Dirty)) {
            memcpy(m_validate_buffer.get(), m->page_addr(other_local_ndx), get_encryption_page_size());
            break;
        }
    }

    if (memcmp(m_validate_buffer.get(), page_addr(local_ndx), get_encryption_page_size()) != 0) {
...
void EncryptedFileMapping::write_barrier(const void* addr, size_t size) noexcept
{
    ...
    while (size > 0) {
        ...
        auto bytes_in_page = uint16_t(std::min<size_t>(get_encryption_page_size(), size) - offset_in_page);
...
void EncryptedFileMapping::set(void* new_addr, size_t new_size, SizeType new_file_offset)
{
    CheckedLockGuard lock(m_file.mutex);
    auto page_size = get_encryption_page_size();
    REALM_ASSERT(new_file_offset % page_size == 0);
    ...
    m_first_page = size_t(new_file_offset / get_encryption_page_size());
    m_page_state.clear();
    m_page_state.resize(new_size / get_encryption_page_size(), PageState::Clean);
}
...
{
    auto page_size = get_encryption_page_size();
    SizeType r = size % page_size;
    size += r ? page_size - r : 0;
