...
void testCompile(String description, dynamic source, dynamic matcher,
    {dynamic skip, void Function(LogRecord)? onLog}) {
    ...
    generate() async {
      ...
      return _formatter.format(
        String.fromCharCodes(
          await testRW.readAsBytes(testRW.testing.assets.first),
        ),
      );
    }
...
void testCompileMany(
    String description, Iterable<dynamic> sources, dynamic matcher) async {
  ...
  matcher = switch (matcher) {
    ...
    Iterable<String> strings =>
      completion(equals(strings.map((e) => _formatter.format(e)))),
    Iterable<File> files =>
      completion(files.map((x) => _formatter.format(x.readAsStringSync()))),
  ...
  test(description, () {
    generate() async {
      ...
      for (final assetId in testRW.testing.assets) {
        result.add(await testRW.readAsBytes(assetId));
      }
