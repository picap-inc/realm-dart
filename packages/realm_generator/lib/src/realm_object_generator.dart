...
Future<ResolvedLibraryResult> _getResolvedLibrary(
    LibraryElement2 library, Resolver resolver) async {
  var attempts = 0;
  while (true) {
    try {
      final freshLibrary =
      ...
      return await freshSession.getResolvedLibraryByElement2(freshLibrary)
...
class RealmObjectGenerator extends Generator {
  ...
  Future<String> generate(LibraryReader library, BuildStep buildStep) async {
    return await measure(
      () async {
        final result =
            await _getResolvedLibrary(library.element, buildStep.resolver);

        return scopeSession(
          result,
          () {
            final codeLines =
                library.classes.realmInfo.expand((m) => m.toCode())..toList();
...
extension on Iterable<ClassElement2> {
